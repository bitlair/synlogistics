...
for invoice in Invoice.objects.filter(date__gte=first_day, date__lte=last_day):
    ...
    print "%s ex btw + %s btw = %s incl btw" % (invoice_ex_btw, invoice_btw, invoice_inc_btw)
    tot_ex_btw += invoice_ex_btw
    tot_inc_btw += invoice_inc_btw
    tot_btw += invoice_btw

print
print "Totaal ex btw: %s" % tot_ex_btw
print "Totaal btw: %s" % tot_btw
print "Totaal incl btw: %s" % tot_inc_btw